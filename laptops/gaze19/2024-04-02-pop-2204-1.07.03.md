...
|---------------|----------------|--------------|------------|-------------------------|
| INCOMPLETE    | Pop!\_OS 22.04 | 1.07.04      | 1.07.03MI1 | Both                    |
...
- Decrypt prompt only appears on USB-C external display, not HDMI
- GDM3 shows up on both, but logging in while the lid is closed seems to lead to a hung state at blank screens. Opening lid doesn't even bring it back, need to REISUB
...
- Suspend problems present. 150 suspends always fails after an indeterminate number of cycles, and system reboots.
- Only reaches C3 power state
- Front LED blink during suspend is different than in the past, but I don't think this is a problem.
...
- TPM errors show up on screen and in logs during suspend/resume cycles.
- SATA drives not supported.
