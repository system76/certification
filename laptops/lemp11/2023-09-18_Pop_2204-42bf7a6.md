...
- DP/USB-C does not prevent login on boot. But, Lemp11 suffers from issues found in a number of systems where devices connected over thunderbolt are not initialized at boot. Despite this, replugging the DP/USB-C display while the system is closed and at the decrypt prompt leads to the system correctly displaying the decrypt prompt on the external display. This is not a regression.
