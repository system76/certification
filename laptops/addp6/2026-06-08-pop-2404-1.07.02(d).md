...
|---------------|----------------|---------------------------------------|------------------------------------|
| INCOMPLETE    | Pop!\_OS 24.04 | 1.07.02(d) (Clevo unmodified version) | 1.09.02 (Clevo unmodified version) |
...
- NOTE: this is the Clevo default hotkey layout. We made changes to these hotkeys, so customized EC with our binding changes will need to be tested separately.
- Touchpad lock hotkey does nothing
- Display brightness hotkeys summon the OSD, but the panel stays at full brightness all the time
- Suspend key is not present on this version (but will be on our customized version)
- Keyboard backlight is stuck on full blue
...
- Decryption screen is not shown on HDMI display, but it is shown on USB-C display
...
- Suspend is broken. Screen locks and turns off, but keyboard backlight and power button light stay on solid. Front status LED never blinks.
- Listed power states are C1, C6S, C10, and C1E. C10 shows ~99%.
