...
| ------------- | -------------- | ------------ | ---------- |
| COMPLETE      | Pop!\_OS 24.04 | f888628      | f888628    |
...
REGRESSION:
  incorrect headset/combojack/mic input detection:
  this system has 2 audio jacks: one for headphones/combo, and another for mic input
    when you plug in a combo set, it should prompt you asking if it is a headset or just headphones
      it does not prompt you if you plug a combo set into the headphonejack
      the issue happens when you plug the headset into the mic input
        it will then ask if this is a combo set or a headphone
        this prompt should happen on the combo-jack input, not the mic input
...
does not reach C8 state after idling for 10 minutes
